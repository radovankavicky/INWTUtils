...
- Your colleagues will like you and are happy to pick your work up where you left
off. Well not quite, but we can get there.
...
decided to make them mandatory. Every time code is checked into a git
repository, a CI-tool will automatically check for any style violations. Some
details can be found below how this is implemented. Another component is a
template for a typical script which can also be found below. We also implemented
...
test. Our own check are:

-	Use <- for assignment, not =
-	Spaces should be after all commas, but not before
-	Operators like +, -, =, … should be surrounded by spaces
-	The line length should not exceed a certain number of characters, i.e., 80 or 100
-	Spaces should be used instead of tabs
-	Object names should not be too long
-	Remove empty blank lines at the end of the code and empty spaces at the end of lines
...
have been chosen by someone else and are not in line to your personal
...
not aware of them anymore. A useful function for this task is `rmPkgs` from the
